...
**SPACE モードの可視化**: REVEAL のシミュレーション後半 (`simT > 1.8s`) で
グリッド全体をヒートマップとして描画する。赤=敵が近い・緑=空いている、
最高 margin に近いほど不透明。真値マーカーが「なぜそこなのか」が直感的に
読める ("どのセルが一番緑か" を見ればよい)。`OpenSpaceResult.cells` を
そのまま `drawSpaceHeatmap` に渡す形なので、Voronoi/PCS 実装に差し替えても
描画側は無変更で済む。
